...
산업용 PLC 클라우드 모니터링 & 제어 플랫폼

---

## 🎯 풀스택 범위

자체 하드웨어 보드 설계(Link) → 클라우드 아키텍처 → 모바일 앱 → 대시보드 → AI 제어(MIND)까지 전 과정 개발
...
### 1. 통신 단절 시 데이터 유실
- **문제**: 공장 환경은 네트워크가 불안정. 연결 끊기면 그 사이 데이터 손실
- **해결**: 로컬 큐잉 시스템 — Link 보드에 버퍼링 → 복구 시 자동 재전송
- **결과**: 네트워크 불안정 환경에서도 데이터 무손실 보장

### 2. 운영 중인 PLC 무중단 업데이트
- **문제**: 24시간 가동 라인, 업데이트 때문에 멈추면 손실
- **해결**: OTA 무중단 업데이트 + 롤백 메커니즘
- **결과**: 라인 가동 중 안전하게 업데이트 배포

---

## 📦 제품 라인업

- **Link**: PLC-클라우드 연결 하드웨어 (자체 설계 PCB)
- **Cloud**: 실시간 데이터 수집 & 스트리밍
- **Dashboard**: 웹 기반 시각화 & 디바이스 관리
- **Mobile**: 언제 어디서나 모니터링 & 제어
- **MIND**: AI 기반 자연어 제어 시스템
